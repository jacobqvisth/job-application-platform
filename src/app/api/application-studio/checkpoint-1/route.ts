...
export async function POST(req: NextRequest) {
  ...
  if (pkg.status !== 'checkpoint_1') {
    return NextResponse.json({ error: `Package is in ${pkg.status} state, expected checkpoint_1` }, { status: 400 });
  }

  await updatePackage(supabase, package_id, user.id, {
    checkpoint_1_edits: edits || {},
    status: 'matching', // Ready for AS2
  });
