(async function () {
  // ─── ATS detection ───────────────────────────────────────────────────────
  function isJobylonPage() {
    return window.location.hostname === 'emp.jobylon.com';
  }

  // Early exit — not a Jobylon page
  if (!isJobylonPage()) return;

  const atsType = 'jobylon';
  const path = window.location.pathname;
  const href = window.location.href;

  // ─── Page type detection ─────────────────────────────────────────────────
  function isApplicationPage() {
    return /\/applications\/jobs\/\d+\/create\/?$/.test(path);
  }

  function isJobDetailPage() {
    return /\/jobs\/\d+/.test(path) && !/\/applications\//.test(path);
  }
  ...
  // Only inject sidebar on relevant pages (not career listing pages)
  if (!isRelevantPage()) return;

  // ─── Field map (mirrors mappers/jobylon.js — inlined for content script) ─
      ...
      fillBtn.addEventListener('click', async () => {
        ...
        chrome.runtime.sendMessage({ type: 'GET_PROFILE' }, async (response) => {
          ...
          for (const field of fields) {
            ...
            // Cover letter (#id_message) is a hidden field wired to a rich text editor — skip
            if (field.selector === '#id_message') {
              rows.push(
                `<div class="jac-field-row"><span class="jac-field-manual">—</span><span>${field.label} (manual: rich text editor)</span></div>`
              );
              continue;
            }
